...
my_samples = list(client.classified_data(SYMBOL))
...
# train_arr, test_arr = train_test_split(items, test_size=0.05)
# train_samples.extend(samples_by_type[k])
# test_samples.extend(test_arr)

# print('Calculate MAs for train samples and prepare features and data')
# for sample in samples_by_type['sell_7']:
#     rec = []
...
#     for ma_price in moving_average(total, 120, sample.from_date, sample.to_date):
#         norm_ma_120 = get_price_percentage(ma_price, sample.max_price, sample.min_price)
#         rec.append(norm_ma_120)
#     for currency in sample.data:
#         norm_p = get_price_percentage(currency.close, sample.max_price, sample.min_price)
#         rec.append(norm_p)
#     sell7_train.append(rec)
