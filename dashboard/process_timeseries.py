...
def main(currency):
    ...
    scaled_data = pd.read_csv(f'loaded_timeseries_{currency.lower()}',
        header=None,
        names=['open', 'close', 'high', 'low', 'time'],
        sep='\t')
    scaler = StandardScaler() # TODO. It's better to use own percentage scaler
    scaled_data = scaler.fit_transform(scaled_data[['open', 'close', 'high', 'low']])
    ...
    #######model.save('eurusd_h4_model.keras')
    # Evaluate the model
    ...
    # Calculate the middle (average) of open, close, high, and low
    actual_middle = np.mean(y_test_inv, axis=1)
    predicted_middle = np.mean(y_pred, axis=1)

    # Prepare dates for x-axis
    dates = pd.date_range(start='2024-08-01', periods=len(actual_middle), freq='H')

    # Plot actual vs predicted middle values
    plt.figure(figsize=(15, 8))

    plt.plot(dates, actual_middle, label='Actual Middle', color='green')
    plt.plot(dates, predicted_middle, label='Predicted Middle', color='blue')

    # Set labels and title
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.title('Actual vs Predicted Middle (Average of Open, Close, High, Low)')
    plt.legend()
    ...
    print("Predicted next 1000 points (open, close, high, low):")
    print(predictions)
    ...
    plt.ylabel('Price')
    plt.title('Predicted Next 1000 Points')
    plt.legend()
    plt.show()
    ...
    rect = plt.Rectangle((0.7, open_price if close_price >= open_price else close_price),
                        candlestick_width,
    ...
    # pos_encoding_layer = PositionalEncoding(sequence_length=sliding_window, d_model=512)
    # # Generate positional encodings
    # pos_encoding = pos_encoding_layer.pos_encoding.numpy()
...
# TODO. Load new data from a client, and run prediction for a next 1000 periods
