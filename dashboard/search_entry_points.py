...
def check(currency, timeframe, precise, model_path, trade_info):
    ...
    print(f'DEBUG: {info}')
    print(f'DEBUG: spread {spread}, {currency[0:3].upper()} news {event1} in {pair1_news_in if pair1_news_in <= 30 else "N/A"} minutes, impact {pair1_impact}')
    print(f'DEBUG: spread {spread}, {currency[-3:].upper()} news {event2} in {pair2_news_in if pair2_news_in <= 30 else "N/A"} minutes, impact {pair2_impact}')

    for _, sim in buy_cos_sim_stream:
        if sim > precise:
            print(
                f'{currency} {timeframe} BUY -> {sim}, spread: {spread}\n' +
                f'   {currency[0:3].upper()} news {event1} in {pair1_news_in if pair1_news_in <= 30 else "N/A"} minutes, impact {pair1_impact}\n'
                f'   {currency[-3:].upper()} news {event2} in {pair2_news_in if pair2_news_in <= 30 else "N/A"} minutes, impact {pair2_impact}'
            )
    for _, sim in sell_cos_sim_stream:
        if sim > precise:
            print(
                f'{currency} {timeframe} SELL -> {sim}, spread: {spread}\n' +
                f'   {currency[0:3].upper()} news {event1} in {pair1_news_in if pair1_news_in <= 30 else "N/A"} minutes, impact {pair1_impact}\n'
                f'   {currency[-3:].upper()} news {event2} in {pair2_news_in if pair2_news_in <= 30 else "N/A"} minutes, impact {pair2_impact}'
            )
...
if __name__ == '__main__':
    ...
    for currency in config['common']['currencies']:
        ...
        for timeframe in config['common']['timeframes']:
            ...
            info = client.get_info(currency)
            check(currency, timeframe, options.precise, options.model, info)
